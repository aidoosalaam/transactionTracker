...
create user expensetracker with password 'p@55w0rd';
create database expensetrackerdb with template=template0 owner=expensetrackerdb;
...
alter table et_transactions add constraint tran_cat_fk
foreign key (category_id) references et_categories(category_id);
alter  table et_transactions add constraint tran_user_fk
foreign key (user_id) references et_users(user_id);
